...
if __name__ == '__main__':
    ...
    with open('/Users/muscle/Desktop/Ours_SemanticRT/configs/ECM.json', 'r') as fp:
        cfg = json.load(fp)

    trainset, val_loader, testset = get_dataset(cfg)
    train_loader = DataLoader(trainset, batch_size=cfg['ims_per_gpu'], shuffle=True, num_workers=cfg['num_workers'],
                              pin_memory=True)

    Class_a = ClassWeight(method='median_freq_balancing')
    print(Class_a.get_weight(train_loader, 13))
